...
const FIELDS = ["farm_id", "planting_batch_id", "production_date", "yield_quantity_kg", "notes"];
const FIELD_LABELS = ["Farm ID", "Planting Batch ID", "Production Date", "Yield (kg)", "Notes"];
const EDIT_FIELDS = [
	"farm_id",
	"planting_batch_id",
	"production_date",
...
const Production = () => {
	...
	const fetchRows = async () => {
		try {
			const res = await fetch("http://localhost:3001/api/production/");
	...
	const saveEdit = async () => {
		try {
			...
			await fetch(`http://localhost:3001/api/production/${editingId}`, {
				method: "PUT",
				headers: { "Content-Type": "application/json" },
	...
	const handleDelete = async (id) => {
		...
		try {
			await fetch(`http://localhost:3001/api/production/${id}`, { method: "DELETE" });
			fetchRows();
		} catch (err) {
			console.error(err);
		}
