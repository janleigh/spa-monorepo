...
const Equipment = () => {
	...
	const saveEdit = async () => {
		try {
			...
			await fetch(`http://localhost:3001/api/equipment/${editingId}`, {
				method: "PUT",
				headers: { "Content-Type": "application/json" },
			...
			<Box className="acct-toolbar">
				...
				>
					{FIELDS.map((field, i) => (
						<TextField
							key={field}
							required={field !== "last_maintenance_date" && field !== "next_maintenance_date"}
					...
					<Button
						variant="contained"
						disabled={!newRow.name || !newRow.farm_id || !newRow.purchase_date || !newRow.status}
						onClick={handleAdd}
