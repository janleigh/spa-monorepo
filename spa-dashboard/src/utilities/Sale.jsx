...
const FIELDS = ["farm_id", "harvest_id", "sale_date", "customer_name", "quantity_kg", "revenue"];
const FIELD_LABELS = [
	"Farm ID",
	"Harvest ID",
	"Sale Date",
...
const Sale = () => {
	...
	const fetchRows = async () => {
		try {
			const res = await fetch("http://localhost:3001/api/sale/");
	...
	const saveEdit = async () => {
		try {
			...
			await fetch(`http://localhost:3001/api/sale/${editingId}`, {
				method: "PUT",
				headers: { "Content-Type": "application/json" },
	...
	const handleDelete = async (id) => {
		...
		try {
			await fetch(`http://localhost:3001/api/sale/${id}`, { method: "DELETE" });
			fetchRows();
		} catch (err) {
			console.error(err);
		}
