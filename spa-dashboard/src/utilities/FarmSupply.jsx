...
const FIELDS = ["farm_id", "supply_id", "quantity_used", "date_used"];
const FIELD_LABELS = ["Farm ID", "Supply ID", "Quantity Used", "Date Used"];
const EDIT_FIELDS = ["farm_id", "supply_id", "quantity_used", "date_used"];
...
const FarmSupply = () => {
	...
	const emptyNew = { farm_id: "", supply_id: "", quantity_used: "", date_used: "" };
	const [newRow, setNewRow] = React.useState(emptyNew);

	const fetchRows = async () => {
		try {
			const res = await fetch("http://localhost:3001/api/farm_supply/");
	...
	const saveEdit = async () => {
		try {
			...
			await fetch(`http://localhost:3001/api/farm_supply/${editingId}`, {
				method: "PUT",
				headers: { "Content-Type": "application/json" },
	...
	const handleDelete = async (id) => {
		if (!window.confirm("Delete this farm supply record?")) return;
		try {
			await fetch(`http://localhost:3001/api/farm_supply/${id}`, { method: "DELETE" });
			fetchRows();
		} catch (err) {
			console.error(err);
		}
				...
				>
					{FIELDS.map((field, i) => (
						<TextField
							...
							size="small"
							type={field === "date_used" ? "datetime-local" : "text"}
							InputLabelProps={field === "date_used" ? { shrink: true } : undefined}
							value={newRow[field]}
