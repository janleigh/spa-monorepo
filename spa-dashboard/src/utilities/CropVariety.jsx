...
const FIELDS = ["name", "description", "ideal_climate", "average_yield_kg_per_hectare"];
const FIELD_LABELS = ["Name", "Description", "Ideal Climate", "Avg Yield (kg/ha)"];
const EDIT_FIELDS = ["name", "description", "ideal_climate", "average_yield_kg_per_hectare"];
...
const CropVariety = () => {
	...
	const fetchRows = async () => {
		try {
			const res = await fetch("http://localhost:3001/api/crop_variety/");
	...
	const saveEdit = async () => {
		try {
			...
			await fetch(`http://localhost:3001/api/crop_variety/${editingId}`, {
				method: "PUT",
				headers: { "Content-Type": "application/json" },
	...
	const handleDelete = async (id) => {
		if (!window.confirm("Delete this crop variety?")) return;
		try {
			await fetch(`http://localhost:3001/api/crop_variety/${id}`, { method: "DELETE" });
			fetchRows();
		} catch (err) {
			console.error(err);
		}
