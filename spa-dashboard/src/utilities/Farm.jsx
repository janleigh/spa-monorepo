...
const FIELD_LABELS = ["Name", "Location"];
const EDIT_FIELDS = ["name", "location"];
...
const Farm = () => {
	...
	const emptyNew = { name: "", location: "" };
	const [newRow, setNewRow] = React.useState(emptyNew);

	const fetchRows = async () => {
		try {
			const res = await fetch("http://localhost:3001/api/farm/");
	...
	const saveEdit = async () => {
		try {
			...
			await fetch(`http://localhost:3001/api/farm/${editingId}`, {
				method: "PUT",
				headers: { "Content-Type": "application/json" },
	...
	const handleDelete = async (id) => {
		...
		try {
			await fetch(`http://localhost:3001/api/farm/${id}`, { method: "DELETE" });
			fetchRows();
		} catch (err) {
			console.error(err);
		}
