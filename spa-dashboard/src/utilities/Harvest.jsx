...
const FIELDS = ["farm_id", "harvest_date", "quantity_kg", "quality", "notes"];
const FIELD_LABELS = ["Farm ID", "Harvest Date", "Quantity (kg)", "Quality", "Notes"];
const EDIT_FIELDS = ["farm_id", "harvest_date", "quantity_kg", "quality", "notes"];
...
const Harvest = () => {
	...
	const emptyNew = { farm_id: "", harvest_date: "", quantity_kg: "", quality: "", notes: "" };
	const [newRow, setNewRow] = React.useState(emptyNew);

	const fetchRows = async () => {
		try {
			const res = await fetch("http://localhost:3001/api/harvest/");
	...
	const saveEdit = async () => {
		try {
			...
			await fetch(`http://localhost:3001/api/harvest/${editingId}`, {
				method: "PUT",
				headers: { "Content-Type": "application/json" },
	...
	const handleDelete = async (id) => {
		...
		try {
			await fetch(`http://localhost:3001/api/harvest/${id}`, { method: "DELETE" });
			fetchRows();
		} catch (err) {
			console.error(err);
		}
				...
				>
					{FIELDS.map((field, i) => (
						<TextField
							...
							size="small"
							type={field === "harvest_date" ? "date" : field === "quantity_kg" ? "number" : "text"}
							InputLabelProps={field === "harvest_date" ? { shrink: true } : undefined}
							value={newRow[field]}
					...
					<Button
						variant="contained"
						disabled={FIELDS.filter((f) => f !== "notes").some((f) => !newRow[f])}
						onClick={handleAdd}
