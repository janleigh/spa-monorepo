...
const FIELDS = ["farm_id", "record_date", "income", "expenses", "net_profit_loss"];
const FIELD_LABELS = ["Farm ID", "Record Date", "Income", "Expenses", "Net Profit/Loss"];
const EDIT_FIELDS = ["farm_id", "record_date", "income", "expenses", "net_profit_loss"];
...
const FinancialRecord = () => {
	...
	const emptyNew = { farm_id: "", record_date: "", income: "", expenses: "", net_profit_loss: "" };
	const [newRow, setNewRow] = React.useState(emptyNew);

	const fetchRows = async () => {
		try {
			const res = await fetch("http://localhost:3001/api/financial_record/");
	...
	const saveEdit = async () => {
		try {
			...
			await fetch(`http://localhost:3001/api/financial_record/${editingId}`, {
				method: "PUT",
				headers: { "Content-Type": "application/json" },
	...
	const handleDelete = async (id) => {
		...
		try {
			await fetch(`http://localhost:3001/api/financial_record/${id}`, { method: "DELETE" });
			fetchRows();
		} catch (err) {
			console.error(err);
		}
