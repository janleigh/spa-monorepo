...
const FIELDS = ["name", "farm_id", "role", "salary", "contact_info"];
const FIELD_LABELS = ["Name", "Farm ID", "Role", "Salary", "Contact Info"];
const EDIT_FIELDS = ["name", "farm_id", "role", "salary", "contact_info"];
...
const Farmer = () => {
	...
	const emptyNew = { name: "", farm_id: "", role: "", salary: "", contact_info: "" };
	const [newRow, setNewRow] = React.useState(emptyNew);

	const fetchRows = async () => {
		try {
			const res = await fetch("http://localhost:3001/api/farmer/");
	...
	const saveEdit = async () => {
		try {
			...
			await fetch(`http://localhost:3001/api/farmer/${editingId}`, {
				method: "PUT",
				headers: { "Content-Type": "application/json" },
	...
	const handleDelete = async (id) => {
		...
		try {
			await fetch(`http://localhost:3001/api/farmer/${id}`, { method: "DELETE" });
			fetchRows();
		} catch (err) {
			console.error(err);
		}
