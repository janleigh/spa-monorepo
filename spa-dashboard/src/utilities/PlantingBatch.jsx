...
const FIELDS = ["farm_id", "tomato_type", "planting_date", "expected_harvest_date"];
const FIELD_LABELS = ["Farm ID", "Tomato Type", "Planting Date", "Expected Harvest Date"];
const EDIT_FIELDS = ["farm_id", "tomato_type", "planting_date", "expected_harvest_date"];
...
const PlantingBatch = () => {
	...
	const emptyNew = { farm_id: "", tomato_type: "", planting_date: "", expected_harvest_date: "" };
	const [newRow, setNewRow] = React.useState(emptyNew);

	const fetchRows = async () => {
		try {
			const res = await fetch("http://localhost:3001/api/planting_batch/");
	...
	const saveEdit = async () => {
		try {
			...
			await fetch(`http://localhost:3001/api/planting_batch/${editingId}`, {
				method: "PUT",
				headers: { "Content-Type": "application/json" },
	...
	const handleDelete = async (id) => {
		if (!window.confirm("Delete this planting batch?")) return;
		try {
			await fetch(`http://localhost:3001/api/planting_batch/${id}`, { method: "DELETE" });
			fetchRows();
		} catch (err) {
			console.error(err);
		}
