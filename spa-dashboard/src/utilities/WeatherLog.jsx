...
const WeatherLog = () => {
	...
	const fetchRows = async () => {
		try {
			const res = await fetch("http://localhost:3001/api/weather_log/");
	...
	const saveEdit = async () => {
		try {
			...
			await fetch(`http://localhost:3001/api/weather_log/${editingId}`, {
				method: "PUT",
				headers: { "Content-Type": "application/json" },
	...
	const handleDelete = async (id) => {
		if (!window.confirm("Delete this weather log?")) return;
		try {
			await fetch(`http://localhost:3001/api/weather_log/${id}`, { method: "DELETE" });
			fetchRows();
		} catch (err) {
			console.error(err);
		}
		...
		<Box>
			<Box className="acct-toolbar">
				...
				>
					...
					<Button
						variant="contained"
						disabled={!newRow.farm_id || !newRow.log_date}
						onClick={handleAdd}
