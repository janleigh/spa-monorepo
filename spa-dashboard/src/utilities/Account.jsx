...
const FIELD_LABELS = ["Name", "Description", "Location"];
const EDIT_FIELDS = ["name", "description", "location"];

const emptyNew = { name: "", description: "", location: "" };
...
const Account = () => {
	...
	const saveEdit = async () => {
		try {
			...
			await fetch(`http://localhost:3001/api/account/${editingId}`, {
				method: "PUT",
				headers: { "Content-Type": "application/json" },
	...
	const handleDelete = async (id) => {
		...
		try {
			await fetch(`http://localhost:3001/api/account/${id}`, {
				method: "DELETE",
			});
			fetchRows();
		} catch (err) {
