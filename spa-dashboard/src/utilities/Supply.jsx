...
const FIELDS = ["name", "description", "unit_cost", "quantity_on_hand"];
const FIELD_LABELS = ["Name", "Description", "Unit Cost", "Qty On Hand"];
const EDIT_FIELDS = ["name", "description", "unit_cost", "quantity_on_hand"];
...
const Supply = () => {
	...
	const emptyNew = { name: "", description: "", unit_cost: "", quantity_on_hand: "" };
	const [newRow, setNewRow] = React.useState(emptyNew);

	const fetchRows = async () => {
		try {
			const res = await fetch("http://localhost:3001/api/supply/");
	...
	const saveEdit = async () => {
		try {
			...
			await fetch(`http://localhost:3001/api/supply/${editingId}`, {
				method: "PUT",
				headers: { "Content-Type": "application/json" },
	...
	const handleDelete = async (id) => {
		...
		try {
			await fetch(`http://localhost:3001/api/supply/${id}`, { method: "DELETE" });
			fetchRows();
		} catch (err) {
			console.error(err);
		}
